...
export default function Certifications() {
  ...
  return (
    <section id="certifications" className="relative px-6 py-24 border-t" style={{ borderColor: 'var(--border)' }}>
      ...
      {/* Certificate Modal - Rebuilt without tailwindcss-animate dependency */}
      {selectedCert && (
        <div
          className="fixed inset-0 z-[1000] flex items-center justify-center p-4 md:p-8 transition-opacity duration-300 bg-black/95 backdrop-blur-xl"
          onClick={() => setSelectedCert(null)}
        >
          <div
            className="relative w-full max-w-4xl h-full flex flex-col items-center justify-center scale-100 transition-transform duration-300"
          ...
          >
            ...
            <div className="absolute top-0 left-0 right-0 p-6 flex items-center justify-between z-20">
              ...
              <button
                onClick={() => setSelectedCert(null)}
                className="p-3 rounded-full bg-white/5 border border-white/10 hover:bg-white/10 transition-all hover:scale-110"
                style={{ color: 'white' }}
              >
                <X size={24} />
              </button>
              ...
              <div className="relative shadow-2xl rounded-lg overflow-hidden border border-white/10">
                <img
                  ...
                  onError={(e) => {
                    ...
                    // Handle broken image here if needed
                  }}
                />
              </div>
            </div>
