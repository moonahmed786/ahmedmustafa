...
export default function DownloadCVButton({ variant }: Props) {
  ...
  if (variant === 'hero') {
    return (
      <button
        onClick={handleClick}
        disabled={loading}
        className="flex items-center gap-3 px-8 py-4 rounded-xl font-semibold transition-all hover:opacity-90 active:scale-95"
        style={{
  ...
  return (
    <button
      onClick={handleClick}
      disabled={loading}
      className="mono text-[10px] tracking-[0.2em] uppercase px-4 py-1.5 rounded-lg border transition-all hover:border-[var(--accent)] hover:text-[var(--accent)] inline-flex items-center gap-2"
